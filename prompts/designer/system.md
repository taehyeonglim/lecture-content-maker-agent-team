...
### 폰트 위계 (Pretendard 단일 — 강의실 뒷자리 본문 44px 최저, PI 2026-05-27 2차 재갱신)
1920×1080 캔버스 기준 px. PI 가 1차 갱신 (40px) 후에도 "본문 폰트 약간만 더 키워야" 지시 → 본문 계열만 +4px 추가 상향. **헤더 계열은 그대로** (PI 가 "헤더 폰트는 이정도면 충분" 확인).

| 위치 | 크기 | 굵기 | 비고 |
|---|---|---|---|
| 표지 메인 제목 | 120px | 900 | 헤더 격 — 변경 X |
| 절 구분 제목 | 100px | 900 | 헤더 격 |
| 마무리 인용 (closing-quote) | 68px | 700 | 헤더 격 |
| 슬라이드 헤딩 · 헤더바 타이틀 · 표지 부제 | 48px | 500-700 | 헤더 격 |
| flow-card step-key | **44px** | 900 | 헤더성 키 (그대로 — 우연히 본문과 같음) |
| **본문 텍스트 (body-text)** | **44px** | **400 (line-height 1.5)** | **최저, +4 상향** |
| 번호 칩 섹션 텍스트 / KRDS 콜아웃 / copy ul | **40px** | 400-500 | **본문 격, +4 상향** |
| 표 td / flex-2col col-card ul | **38px** | 400 | **본문 격, +4 상향** |
| 표 th / 표지 eyebrow / 마무리 meta | 32px | 700 | 헤더 격 |
| 라벨 / 슬라이드 부제 / 헤더 chapter-chip | 28px | 500-700 | 라벨 격 |
| 캡션·각주 / figcaption | 24px | 400 | text-muted |
| 헤더바 메타 / 페이지 번호 | 22px | 500 | text-muted |
| 콜아웃 "요약" 칩 | 18px | 700 | 마이크로 |
| 사진 credit | 12px | 400 | 마이크로 |

핵심 원칙: **본문(정보 전달 텍스트)은 헤더(구조 표시) 보다 작아도 가독성 충분해야 함**. 헤더는 시선 가이드, 본문은 실제 읽기 대상. PI 정책상 본문 44px 최저.
